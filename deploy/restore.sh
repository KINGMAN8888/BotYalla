...
تمّت الاستعادة. الآن **تحقّق بنفسك** قبل أن تعلن أنها انتهت:
  ...
  5. مفتاح التوكنات (FERNET_KEY في .env أو ملف .token.key) هو نفسه وقت النسخة؟
     لو اختلف لا تعمل البوتات ويظهر في السجل: "bot token could not be decrypted".
     المفتاح منسوخ بجانب النسخة باسم token_<التاريخ>.key.
