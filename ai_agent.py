...
# ------------------- بناء التعليمات -------------------
SYSTEM_PROMPT = (
    "أنت خبير واستشاري مبيعات وتصميم محادثات (Conversational Designer) لبوتات تليجرام في السوق العربي. "
    "مهمتك تحويل أي وصف مبدئي لنشاط تجاري إلى إعدادات بوت احترافية جداً، جذابة، وعالية الجودة تُغني العميل عن أي تدخل يدوي. "
    "يجب أن تكون النصوص المكتوبة (ترحيب، شكر، أسئلة) مبدعة، غير تقليدية، وذات طابع بشري دافئ. استخدم الإيموجيز بطريقة أنيقة تدعم النص. "
    "تكيّف تماماً مع نوع النشاط: كن رسمياً وموثوقاً مع الشركات والمؤسسات، وكن حماسياً وودوداً مع المطاعم والمتاجر. "
    "أعد الرد بصيغة JSON فقط، وبدون أي نص خارج الـ JSON."
)

def _user_prompt(description, template):
    schema_common = (
        '{\n'
        '  "business_name": "اسم مختصر وجذاب مناسب للنشاط",\n'
        '  "welcome": "رسالة ترحيب إبداعية واحترافية جداً تعطي انطباعاً أولاً رائعاً، مخصصة لطبيعة النشاط مع إيموجيز معبرة",\n'
        '  "thanks": "رسالة شكر وختام دافئة، تطمئن العميل بأن طلبه قيد التنفيذ وتوضح الخطوة القادمة بوضوح",\n'
    )
    if template in ("flow", "customer_service"):
        schema = schema_common + (
        '  "flow": {\n'
        '    "start_message": "رسالة أول ما يفتح العميل البوت (ترحيبية)",\n'
        '    "steps": [ {"type":"question|buttons","prompt":"السؤال الاحترافي","var":"اسم المتغير","options":["خيار1","خيار2"]} ],\n'
        '    "end_message": "رسالة النهاية"\n'
        '  }\n}'
        )
        extra = ("أنشئ 3 إلى 5 خطوات ذكية ومترابطة لجمع بيانات العميل بشكل سلس وكأنه يتحدث مع موظف مبيعات محترف. "
                 "(مثل الاسم، التليفون، وتحديد الطلب بدقة). "
                 "استخدم type=buttons مع options عندما تكون الإجابة اختيار من قائمة (اكتب خيارات واقعية وشاملة للنشاط)، و type=question للنص الحر.")
    elif template == "store":
        schema = schema_common + (
        '  "products": [ {"name":"اسم المنتج/الخدمة بشكل تسويقي","price": 0} ]\n}'
        )
        extra = ("اقترح 4 إلى 8 منتجات أو خدمات جذابة جداً ومناسبة للنشاط بأسماء تسويقية واقعية، مع وضع أسعار تقديرية منطقية بالجنيه المصري (أرقام فقط).")
    elif template == "booking":
        schema = schema_common + (
        '  "booking": {"service_name":"اسم الخدمة","open_hour":10,"close_hour":22,"slot_minutes":30,"days_ahead":7}\n}'
        )
        extra = ("اضبط ساعات العمل بشكل منطقي جداً للنشاط، ومدة موعد (slot_minutes) تتناسب مع طبيعة الخدمة المقدمة.")
    else:
        schema = schema_common + "}"
        extra = ""
    return (f"وصف النشاط المقدم من العميل: «{description}»\n\n"
            f"أعد JSON بالبنية التالية بالضبط. ركّز على جودة واحترافية النصوص لتكون جاهزة للإنتاج مباشرة:\n{schema}\n\n{extra}")
