...
# الزرّ الأول هو الخطوة التالية لا سؤالٌ عن المنصة: 143 محادثة في يومين أعطت 10
# تسجيلات فقط، لأن المحادثة كانت تنتهي بشرح لا بخطوة.
STARTERS = {
    "ar": ["اعملّي بوت لنشاطي", "الباقات والأسعار", "إيه هي BotYalla؟"],
    "en": ["Build a bot for my business", "Plans & pricing", "What is BotYalla?"],
}

# سطر الإقفال: سؤال واحد إجابته كلمة، يُلحق بكل رد لا يطلب شيئاً بالفعل.
CLOSER = {
    "ar": "\n\nقولّي اسم نشاطك وإيه اللي بتبيعه، وأنا أقولك الخطوة الجاية بالظبط 👇",
    "en": "\n\nTell me your business name and what you sell, and I'll give you the exact "
          "next step 👇",
}
# نيّات لا تُقفَل: من يطلب موظفاً أو يسأل عن الشغل جوابه هو الخطوة نفسها.
NO_CLOSE = ("human", "work")


def closer(lang="ar"):
    return CLOSER.get(lang, CLOSER["ar"])
...
def facts():
    ...
    return {
        ...
        "features": [
            ...
            "متجر تليجرام: بطاقة منتج بصورة وسعر ووصف، وشحن بسعر موحّد أو لكل منطقة مع توصيل مجاني فوق مبلغ",
            "إضافة «منتجاتي من تليجرام» (99ج شهرياً لكل بوت): صاحب المتجر يضيف منتجاته من داخل بوته "
        ...
        "sales_notes": [
            ...
            "**اقفل كل رد بخطوة واحدة.** لا تنهِ رسالة بشرح فقط: إما تسأل عن اسم النشاط وما "
            "يبيعه، وإما تعطي رابط التسجيل، وإما تعرض أن يجهّزه الفريق له. سؤال واحد في "
            "الرسالة لا ثلاثة.",
            "أول ما تعرف نشاطه وقناته: احفظه بأداة lead (الاسم + النشاط + القناة)، وقُل له إننا "
            "نقدر نجهّزله بوت تجربة، وابعت رابط التسجيل. لا تنتظر أن يطلب هو.",
...
def offline_reply(text, lang="ar"):
    ...
    كل رد يُقفَل بسؤال واحد عن اسم النشاط (`CLOSER`) إلا نيّات `NO_CLOSE`. الرد
    الذي ينتهي بشرحٍ جميل ولا يطلب شيئاً هو سبب أن 143 محادثة أعطت 10 تسجيلات."""
    text_out, btns = _offline_body(text, lang)
    tail = closer(lang)
    if _intent(text) not in NO_CLOSE and tail.strip() not in text_out:
        text_out += tail
    return text_out, btns


def _offline_body(text, lang="ar"):
    f = facts()
    en = lang == "en"
